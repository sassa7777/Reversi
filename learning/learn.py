...
diagonal6_idx = [[2, 11, 20, 29, 38, 47], [16, 25, 34, 43, 52, 61], [5, 12, 19, 26, 33, 40], [23, 30, 37, 44, 51, 55]]
...
diagonal4_idx = [[4, 13, 22, 31], [32, 41, 50, 59], [3, 10, 19, 24], [39, 46, 53, 60]]
...
corner_3x3_idx = [[0, 1, 8, 9, 2, 16, 10, 17, 18], [0, 8, 1, 9, 16, 2, 17, 10, 18], [7, 6, 15, 14, 5, 23, 13, 22, 21], [7, 15, 6, 14, 23, 5, 22, 13, 21], [56, 57, 48, 49, 58, 40, 50, 39, 38], [56, 48, 57, 49, 40, 58, 39, 50, 38], [63, 62, 55, 54, 61, 47, 53, 46, 45], [63, 55, 62, 54, 47, 61, 46, 53, 45]]

edge_x_side_idx = [[32, 24, 16, 8, 0, 9, 1, 2, 3, 4], [4, 3, 2, 1, 0, 9, 8, 16, 24, 32], [39, 31, 23, 15, 7, 14, 6, 5, 4, 3], [3, 4, 5, 6, 7, 14, 15, 23, 31, 39], [24, 32, 40, 48, 56, 49, 57, 58, 59, 60], [60, 59, 58, 57, 56, 49, 48, 40, 32, 24], [59, 60, 61, 62, 63, 54, 55, 47, 39, 31], [31, 39, 47, 55, 54, 63, 54, 62, 6, 60, 59]]

edge_block_idx = [[0, 2, 3, 10, 11, 12, 13, 4, 5, 7], [7, 23, 31, 22, 30, 38, 46, 39, 47, 63], [56, 58, 59, 50, 51, 52, 53, 60, 61, 63], [0, 16, 24, 17, 25, 33, 41, 32, 30, 56]]
for pattern in deepcopy(edge_block_idx):
    edge_block_idx.append(list(reversed(pattern)))

pattern_idx = [diagonal8_idx, diagonal7_idx, diagonal6_idx, diagonal5_idx, diagonal4_idx, edge_2x_idx, h_v_2_idx, h_v_3_idx, h_v_4_idx, corner_3x3_idx, edge_x_side_idx, edge_block_idx]
...
ys = []
names = ['diagonal8_idx', 'diagonal7_idx', 'diagonal6_idx', 'diagonal5_idx', 'diagonal4_idx', 'edge_2x_idx', 'h_v_2_idx', 'h_v_3_idx', 'h_v_4_idx', 'corner_3x3_idx', 'edge_x_side_idx', 'edge_block_idx']
idx = 0
for i in range(len(pattern_idx)):
    layers = []
...
model.compile(loss='mse', metrics=['mae'], optimizer=tf.keras.optimizers.Adam(learning_rate=0.1))
...
history = model.fit(train_data, train_labels, epochs=n_epochs, validation_data=(test_data, test_labels), callbacks=[early_stop, lr_callback, tensorboard_callback], batch_size=64)
