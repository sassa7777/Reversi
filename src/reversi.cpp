...
//    }
    ...
    cout << "Score : " << score << endl;
    cout << "Score(stone) : " << ((afterIndex < 60) ? score/1000000000000000 : score) << endl;
    return 1;
}
...
int64_t search_nega_scout(uint64_t playerboard, uint64_t opponentboard, bool hint) {
    ...
    for (search_depth = max(1, end_depth-4); search_depth <= end_depth; ++search_depth) {
        ...
        sort(execution::unseq, moveorder.begin(), moveorder.end());
        alpha = MIN_INF;
        beta = MAX_INF;
...
int64_t nega_scout_finish(int64_t alpha, int64_t beta, uint64_t playerboard, uint64_t opponentboard, uint64_t legalboard) {
    ...
    if (stones < 47) {
        while(legalboard) {
            ...
            moveorder[count].score = evaluate_moveorder(moveorder[count].opponentboard, moveorder[count].playerboard);
            ++count;
        }
    } else {
        while(legalboard) {
