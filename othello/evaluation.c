...
bool anycorner(char board[10][10]) {
    if (board[1][1] != 0 || board[1][8] != 0 || board[8][1] != 0 ||
        board[8][8] != 0) return true;
    return false;
}
...
int countscore(char board[10][10], int *turn, bool canput[10][10],
    ...
    if(*turn >= 60)
    {
        ...
        else score += 100*score_countstone(board);
    }
    score += 2 * score_stone(board);
    score += 10 * score_fixedstone(board);
    if (*playerrn == 2) score += score_putable(board, canput);
    if (*playerrn == 1) score -= score_putable(board, canput);
//    if(anycorner(board) == false) {
//        score += 1*score_mountain(board);;
//        score += 1*score_halfblock(board);
//        score += 1*score_bomb(board);
//    }
    //    else if(*turn <= 40) {
    //        //score += 1*score_mountain(board);
    //        //score += 1*score_pureblock(board);
    //        //score += 1*score_purewing(board);
    //        //score += 1*score_halfblock(board);
    //        score += 2*score_goodcorner(board);
    //    }
    //    else if(*turn <= 50) {
    //        score += 1*score_goodcorner(board);
    //    }
    return score;
}
